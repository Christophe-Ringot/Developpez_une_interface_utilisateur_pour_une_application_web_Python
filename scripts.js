/* class carousel (défilement des films par catégories) */
...
class Carousel
{
    ...
    fillCategoryMovies(pictures)
    {
        ...
        for (i = 0; i < 7; i++)
        {
            this.categoryMovies.push(extendedPictures(4 * i, 4 * i + 4))
        }
    }

    fillAllModals(modals)
    {
        let extendModals = modals.concat(modals, modals, modals)
        var x
        for (x= 0; x < 7; x++)
        {
            this.categoryModals.push(extendModals.slice(4 * x, 4 * x + 4))
        }
    }
    ...
    fillMoviesList(movieCategory)
    {
        ...
        let displayMovies = this.categoryMovies[0]
        ...
        this.firstMovie.src = displayMovies[0]
        this.secondMovie.src = displayMovies[1]
        this.thirdMovie.src = displayMovies[2]
        this.fourthMovie.src = displayMovies[3]
    }

    moveRight()
    {
        this.index ++
        if (this.index > this.categoryMovies.length) {
            this.index = 0
        }
